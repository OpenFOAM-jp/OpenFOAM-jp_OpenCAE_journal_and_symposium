...
Foundation版の開発はGithubで行われている． \cite{URL:GitHub-foundation}Foundation版ではissueの管理はGithubでは行わないので注意が必要である．バグ報告はHP\cite{URL:openfoam.com}に併設されている専用のissueTrackingのページへ登録する必要がある．
